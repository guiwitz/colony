...
class Analysis:
    ...
    def on_update_folder(self, change):
        ...
        experiment = pd.concat(
            [
                pd.DataFrame(
                    pd.read_pickle(x.joinpath("Result/results.pkl").as_posix())
                ).T
        ...
        experiment["exp_name"] = experiment.filename.apply(
            lambda x: re.findall(".*?_(.*)?-\d_d\d.*", x)[0]
        )
        experiment["repl_ind"] = experiment.filename.apply(
            lambda x: int(re.findall(".*-(\d)_d\d.*", x)[0])
        )
        experiment["day"] = experiment.filename.apply(
            lambda x: int(re.findall(".*-\d_d(\d).*", x)[0])
        )
    ...
    def plotting(self, change=None):
        ...
        with self.out:
            ...
            for x in subsel.index:
                plt.plot(
                    subsel.loc[x]["contour"][:, 1] - subsel.loc[x]["center_mass"][1],
                    subsel.loc[x]["contour"][:, 0] - subsel.loc[x]["center_mass"][0],
                    "k",
                )
