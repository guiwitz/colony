...
class Gui:
    def __init__(self):
        ...
        self.runall_button = ipw.Button(
            description="Analyze selected folder (single day)",
            style=style,
            layout=layout,
        )
        ...
        self.runexp_button = ipw.Button(
            description="Analyze full experiment (multi-folder)",
            style=style,
            layout=layout,
        )
        ...
        self.save_plots_check = ipw.Checkbox(
            description="Save plots during analysis", value=True
        )

        self.zip_button = ipw.Button(
            description="Zip results", style=style, layout=layout
        )
    ...
    def analyse_single(self, b):
        ...
        if self.save_plots_check:
            self.plot_result()
            self.fig.savefig(
                self.folder_sel.cur_dir.as_posix() + "/Result/" + file + "_seg.png"
            )
    ...
    def analyse_experiment(self, b):
        ...
        for indf, f in enumerate(folder_list):
            ...
            self.results = {}
            current_files = [os.path.split(x)[1] for x in f.glob("*.jpg")] + [
            ...
            for ind, file in enumerate(current_files):
                ...
                with self.out:
                    ...
                    if self.save_plots_check:
                        ...
                        self.plot_result()
                        self.fig.savefig(
                            self.folder_sel.cur_dir.as_posix()
                            + "/Result/"
                            + file
                            + "_seg.png"
                        )
        ...
        with self.out:
            ...
            if self.results[current_file]["contour"] is not None:
                image = skimage.io.imread(
                    self.folder_sel.cur_dir.as_posix() + "/" + current_file
                )
    ...
    def save_results(self, b=None):
        ...
        pd.DataFrame(
            [
                {
                    ...
                    if self.results[x]["peaks"] is not None
                    else None,
            ...
            self.folder_sel.cur_dir.as_posix() + "/Result/summary.csv", index=False
        )
    ...
    def save_all_plots(self, b):
        ...
        for f in self.select_file.options:
            ...
            self.plot_result()
            self.fig.savefig(
                self.folder_sel.cur_dir.as_posix() + "/Result/" + f + "_seg.png"
            )
        ...
        with self.out:
            ...
            subprocess.call(
                [
                    "tar",
                    "-czf",
                    self.folder_sel.cur_dir.parent.as_posix() + "/to_download.tar.gz",
                    "-C",
                    self.folder_sel.cur_dir.parent.as_posix(),
                    self.folder_sel.cur_dir.name,
                ]
            )
